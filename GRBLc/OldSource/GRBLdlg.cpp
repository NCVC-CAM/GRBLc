...
CGRBLdlg::CGRBLdlg(NCVCHANDLE hDoc) : CDialogEx(IDD_DIALOG1, NULL)
	,m_eventQuery(FALSE, TRUE)		// Manual event
{
	m_hDoc = hDoc;
	m_pCom = NULL;
...
UINT CGRBLdlg::CycleStartThreadFunc(LPVOID pParam)
{
	...
	static	sregex	reMcode = 'M' >> (s1= +_d) >> (s2= *_);
#ifdef _DEBUG
	...
	CGRBLdlg* pDlg = reinterpret_cast<CGRBLdlg*>(pParam);
	CGRBLcOption* pOpt = AfxGetGRBLcApp()->GetOption();
	int		i, nowTraceObj, nLoop = NCVC_GetNCBlockDataSize(pDlg->m_hDoc);
	char	szBlock[MAXREGBUF];
	CString	strLine;
	string	strBlock;
	smatch	what;
	NCDATA	nc;		nc.dwSize = sizeof(NCDATA);		// NCVCdefine.h
	BOOL	bTrace, bM30 = false;
	...
	if ( pOpt->GetIntOpt(grblI_WithTrace) ) {
		...
		nowTraceObj = NCVC_TraceNextDraw(pDlg->m_hDoc);
		bTrace = NCVC_GetNCData(pDlg->m_hDoc, nowTraceObj, &nc);
	}
	else
		bTrace = false;

	for ( i=0; i<nLoop && pDlg->m_bCycleThread && !bM30; i++ ) {
		NCVC_GetNCBlockData(pDlg->m_hDoc, i, szBlock, sizeof(szBlock));
		strBlock = szBlock;
		...
		if ( regex_match(strBlock, what, reOBS) ) {
			...
			if ( pOpt->IsOBScheck(nOBS) ) {
				pDlg->AddMessage("Optional Block Skip \""+strBlock+"\"", msgInfo);
				continue;
			}
			strBlock = what[2].str();
		}
...
//				break;
		...
		// NCVC trace
		while ( bTrace && pDlg->m_bCycleThread && nc.nLine+1<i ) {
			NCVC_TraceStart(pDlg->m_hDoc);
			nowTraceObj = NCVC_TraceNextDraw(pDlg->m_hDoc) - 1;
			bTrace = NCVC_GetNCData(pDlg->m_hDoc, nowTraceObj, &nc);
		}
		// Check Grbl
		do {
			Sleep(pOpt->GetIntOpt(grblI_QueryTime));
		} while ( pDlg->m_bCycleThread && pDlg->m_statGRBL==grblRUN );
		pDlg->m_eventCycleRecv.Lock();		// wait response
		switch ( pDlg->m_statGRBL ) {
		case grblALARM:
			pDlg->m_bCycleThread = false;	// thread end
			break;
		case grblHOLD:
		case grblDOOR:
			pDlg->m_bCycleThreadSuspend = true;
			pDlg->m_pCycleThread->SuspendThread();
			break;
		}
		// Send Grbl
...
//			tSpan = te - ts;
			strLine.Format("Line=%d/%d %s", i, nLoop, strBlock.c_str());
			pDlg->m_lcSendList.SetItemState(i, LVIS_SELECTED|LVIS_FOCUSED, LVIS_SELECTED|LVIS_FOCUSED);
			pDlg->m_lcSendList.EnsureVisible(i, false);
//			pDlg->m_stLine.SetWindowText(strLine+tSpan.Format(" %H:%M:%S passed"));
			pDlg->m_stLine.SetWindowText(strLine);
			pDlg->m_strSend = strBlock;
			if ( pDlg->SendCommand(strBlock) ) {
				if ( pDlg->m_bSingleLock ) {
					pDlg->m_eventSingle.Lock();
				}
			}
			else {
				pDlg->m_bCycleThread = false;
			}
		}
	}

	pDlg->SendMessage(WM_CYCLE_END);
	if ( pDlg->m_bCycleThread ) {
		te = CTime::GetCurrentTime();
		tSpan = te - ts;
